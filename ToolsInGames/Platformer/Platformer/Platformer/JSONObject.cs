...
namespace Platformer
{
     ...
     * 
     * Adapted by Moritz Wundke for MICTTs Kinect Challenge 2013
     */
                    ...
                    else
                    {
                        ...
                        catch (System.FormatException)
                        {
                            ...
                            switch (str[0])
                            {
                                ...
                                default:
                                    type = Type.NULL;
                                    Console.WriteLine("improper JSON formatting:" + str);
                                    return;
                            }
                            int depth = 0;
                            ...
                            for (int i = 1; i < str.Length; i++)
                            {
                                ...
                                if (depth == 0 && !openquote)
                                {
                                    if (str[i] == ':' && !inProp)
                                    {
                                        ...
                                        catch { Console.WriteLine(i + " - " + str.Length + " - " + str); }
                                        token_tmp = i;
                                    }
                                    if (str[i] == ',')
                                    {
            ...
            if (type == Type.OBJECT)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    switch (val.type)
                    {
                        ...
                        default: Console.WriteLine("Omitting object: " + (string)keys[i] + " in dictionary conversion"); break;
                    }
                }
                return result;
            }
            else Console.WriteLine("Tried to turn non-Object JSONObject into a dictionary");
            return null;
        }
    }
}